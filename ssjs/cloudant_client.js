...
cloudant_client.getAnnotations = function(spaceId, start, limit) {
    ...
    return new Promise((resolve, reject) => {
        cloudant_client.db.find(
            {
                selector: {
                    _id: {
                        '$regex': "^ant_*"
                    },
                    spaceId: cloudant_client.parseDocId(spaceId, 'SPC'),
                },
                limit: lst,
                skip: skp
            },
...
cloudant_client.getMoments = function(spaceId, start, limit) {
    let skp = (start ? start : 0);
    let lst = (limit ? limit : 100);
   // console.log('>>>>> query cloudant');
    return new Promise((resolve, reject) => {
        //try {
            cloudant_client.db.find(
                {
                ...
                (err, result) => {
                    ...
                    //console.log('got docs: ' + JSON.stringify(result.docs));
                    resolve(result.docs);
                },
            );
        /*} catch (e) {
...
cloudant_client.getMessagesBetween = function(spaceId, startDate, endDate, start, limit) {
    let skp = (start ? start : 0);
    let lst = (limit ? limit : 100);
     return new Promise((resolve, reject) => {
        //try {
