...
<<<<<<< HEAD
		...
		get: function(request, response) {
			...
			if (spc) {
				...
				response.status(200);
				response.setHeader('Content-Type', 'application/json');
				response.write(JSON.stringify(spc));
				response.end();
			} else {
				// STUB - ERROR
				// TODO: should have load from Cloudant
				response.status(500);
				response.setHeader('Content-Type', 'application/json');
				response.write('{"status" : "Error: Space does not exist!"}');
				response.end();
			}
		},
		_loadSpaceIntoCoach: function(id) {
		...
		_loadNextMessages: function(spaceId, startCursor) {
		...
		_loadNextUsers: function(spaceId, startCursor) {
			if (startCursor) {
				return new Promise((resolve, reject) => {
		...
		messages: {
			get: function(request, response) {
				...
				let skip = +request.query.start;
				let limit = +request.query.limit;
				let sDate = +request.query.startDate;
				let eDate = +request.query.endDate;
				if (sDate && eDate) {
					cloudant.getMessagesBetween(spaceId, sDate, eDate, skip, limit).then((result) => {
						console.log("--------> Found %d documents", result.length);
						// return results
						response.status(200);
						response.setHeader('Content-Type', 'application/json');
						...
						response.status(500);
						response.setHeader('Content-Type', 'application/json');
						response.write('{"status" : "ERROR WHILE LOOKING UP ANNOTATIONS!!!"}');
						response.end();
					});
					return;
				}
				response.status(200);
				response.setHeader('Content-Type', 'application/json');
				response.write('{"status": "Not implemented yet"}');
				response.end();
			},
		},
		annotations: {
			get: function(request, response) {
				...
				let skip = +request.query.start;
				let limit = +request.query.limit;
				cloudant.getAnnotations(spaceId, skip, limit).then((result) => {
					console.log("--------> Found %d documents", result.length);
					// return results
					response.status(200);
					response.setHeader('Content-Type', 'application/json');
			...
			moments: {
				get: function(request, response) {
					...
					cloudant.getMoments(spaceId, skip, limit).then((result) => {
						console.log("--------> Found %d documents", result.length);
						// return results
						response.status(200);
						response.setHeader('Content-Type', 'application/json');
